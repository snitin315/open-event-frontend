...
module('Integration | Helper | not-includes', function(hooks) {
  ...
  test('it works', async function(assert) {
    ...
    await render(
      hbs`{{if (not-includes targetString testString) 'the target does not have the test string' 'the target has the test string'}}`
    );
  ...
  test('it fails graciously', async function(assert) {
    await render(
      hbs`{{if (not-includes 122 44) 'the target does not have the test string' 'the target has the test string'}}`
    );
    assert.equal(this.element.textContent.trim(), 'the target does not have the test string');

    await render(
      hbs`{{if (not-includes null null) 'the target does not have the test string' 'the target has the test string'}}`
    );
    assert.equal(this.element.textContent.trim(), 'the target does not have the test string');
  });
});
