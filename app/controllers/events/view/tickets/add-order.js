...
export default Controller.extend({
  hasTicketsInOrder: computed('model.tickets.@each.orderQuantity', function () {
    return (
      sumBy(this.get('model.tickets').toArray(), (ticket) =>
        ticket.getWithDefault('orderQuantity', 0)
      ) > 0
    );
  }),
    ...
    async placeOrder() {
      ...
      try {
        ...
        await order
          ...
          .then((order) => {
            this.notify.success(
              this.l10n.t('Order details saved. Please fill further details within 10 minutes.')
            );
