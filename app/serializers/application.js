...
export default JSONAPISerializer.extend(EventRelationMixin, {
  ...
  serializeHasMany(snapshot, json, relationship) {
    if (
      ...
      (snapshot.hasMany(relationship.key) && relationship.options.readOnly)
    ) {
      return;
    }
    this._super(...arguments);
  ...
  serializeBelongsTo(snapshot, json, relationship) {
    if (
      !snapshot.belongsTo(relationship.key) ||
      (snapshot.belongsTo(relationship.key) && !snapshot.belongsTo(relationship.key).id)
    ) {
      return;
    }
    this._super(...arguments);
